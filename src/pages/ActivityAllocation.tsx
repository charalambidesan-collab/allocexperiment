...
export default function ActivityAllocation() {
  ...
  useEffect(() => {
    document.title = "Activity Allocation | Cost Allocation Tool";
  ...
  return (
    <PageLayout>
      <main className="p-6">
        ...
        {selectedTower && (
          <ServiceAllocationMatrix initialTower={selectedTower} startAt="allocate" />
        )}
        ...
        {!selectedTower && (
          <section className="rounded-2xl border border-border bg-card shadow-lg p-6 w-full">
            <div className="text-center text-muted-foreground text-xs py-12">
              Select a department from the home page to begin allocating expenses to activities
            </div>
          </section>
        )}
        </div>
